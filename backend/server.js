...
/**
 * API: Truy vấn nhiệt độ theo giờ
 * GET /api/query/temperature?hours=6
 */
app.get("/api/query/temperature", async (req, res) => {
  try {
    ...
    console.error("❌ Lỗi truy vấn nhiệt độ:", error);
    res.status(500).json({ error: error.message });
  }
});

/**
 * API: Truy vấn tỷ giá trung bình theo khoảng thời gian
 * GET /api/query/exchange?base=USD&target=VND&hours=12
 */
app.get("/api/query/exchange", async (req, res) => {
  try {
    const base = req.query.base || "USD";
    const target = req.query.target || "VND";
    ...
    const data = await query(db, sql, [base, target]);
    ...
    console.error("❌ Lỗi truy vấn tỷ giá:", error);
    res.status(500).json({ error: error.message });
  }
});
...
app.get("/api/query/stats", async (req, res) => {
  try {
    // Đếm tổng số records
    const weatherCount = await query(
      db,
      "SELECT COUNT(*) as count FROM weather"
    );
    const exchangeCount = await query(
      db,
    ...
    const logCount = await query(db, "SELECT COUNT(*) as count FROM logs");

    // Lấy dữ liệu mới nhất
    const latestWeather = await query(
      db,
      `
    ...
    const latestExchange = await query(
      db,
      `
    ...
    // Thống kê 24h qua
    const weatherLast24h = await query(
      db,
      `
    ...
    const exchangeLast24h = await query(
      db,
      `
      SELECT COUNT(*) as count 
      FROM exchange 
    ...
    res.json({
      ...
      stats: {
        total: {
          weather: weatherCount[0].count,
          exchange: exchangeCount[0].count,
          messages: messageCount[0].count,
          logs: logCount[0].count,
        },
        last24h: {
          weather: weatherLast24h[0].count,
          exchange: exchangeLast24h[0].count,
        },
        latest: {
          weather: latestWeather[0] || null,
          exchange: latestExchange[0] || null,
        },
      },
    });
  } catch (error) {
...
app.get("/api/query/recent", async (req, res) => {
  try {
    ...
    const recentWeather = await query(
      db,
      `
      SELECT 
        'weather' as type,
        temperature || '°C, ' || humidity || '%' as data,
        created_at
      FROM weather 
      ORDER BY created_at DESC 
      LIMIT ${limit}
    ...
    const recentExchange = await query(
      db,
      `
      SELECT 
        'exchange' as type,
        base_currency || '/' || target_currency || ': ' || rate as data,
        created_at
      FROM exchange 
      ORDER BY created_at DESC 
      LIMIT ${limit}
